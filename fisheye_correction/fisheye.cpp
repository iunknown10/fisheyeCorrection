...
private:
	...
	void generateMap()
	{
		...
		/*cv::Mat projective_trans =
			(cv::Mat_<float>(3, 3) <<
			0.7165, -0.76277, 118.9178,
			0, 0.5839, 21.9616,
			0, -0.00182, 1);
		projective_trans = projective_trans.inv();*/
		...
		for (int h = 0; h < kHeight_; h++)
			...
			{
			float radius_in_project = sqrt((h - kCenterY_)*(h - kCenterY_) + (w - kCenterX_)*(w - kCenterX_)) ;
			...
			float x = ((w - kCenterX_) *(radius_in_fisheye / radius_in_project) );/// pow(0.3 / kHeight_*h + 0.7,2)
			float y = (h - kCenterY_)*(radius_in_fisheye / radius_in_project) ;
...
int main(int argc, char* argv[])
{
	...
	cv::imshow("correct", correct_frame);
	cv::waitKey(0);
	cv::Mat perspectiveTrans = cv::getPerspectiveTransform(original_points, correct_points);
